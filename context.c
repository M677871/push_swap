...
/*   Updated: 2026/01/03 13:15:17 by miissa           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	handle_flag(const char *arr, t_ctx *ctx, int *set)
{
	...
	if (ft_strncmp(arr, "--simple", 9) == 0)
		return (set_strategy(ctx, set, START_SIMPLE) ? 1 : -1);
	if (ft_strncmp(arr, "--medium", 9) == 0)
		return (set_strategy(ctx, set, START_SIMPLE) ? 1 : -1);
	if (ft_strncmp(arr, "--complex", 10) == 0)
		return (set_strategy(ctx, set, START_COMPLEX) ? 1 : -1);
	if (ft_strncmp(arr, "--adaptive", 9) == 0)
		return (set_strategy(ctx, set, START_ADAPTIVE) ? 1 : -1);
	return (0);
}
